var appId = 'app://s-vid.player';

navigator.mozApps.mgmt.getAll().then(function (apps) {
    var appInstalled = apps.some(function (app) {
        return app.origin === appId;
    });
    ...
    if (appInstalled) {
        ...
        function timeLine() {
            ...
            xhr.onload = function () {
                ...
                for (var i = 0; i < array.length; i++) {
                    // app.innerHTML += '<a onclick="" class="focusable" tabindex="' + i + '"><img src="icons/folder.png" width="15px"> ' + array[i].innerHTML.split('<b>')[1].split('</b>')[0] + '</a>';
                    var elem = document.createElement('div');
                    elem.tabIndex = i;
                    elem.className = 'focusable';
                    elem.innerHTML = '<img src="icons/folder.png" width="15px"> ' + array[i].innerHTML.split('<b>')[1].split('</b>')[0];
                    elem.addEventListener('click', function () {
                        showLoader();
                        movieDetails(+'"' + linksarray[i] + '"');
        ...
        function keydownmenubar(e) {
            switch (e.key) {
                ...
                case "ArrowDown":
                    ...
                    break;
                case "Enter":
                    if (document.activeElement.innerHTML != 'Quit') {
                        if (navigator.onLine) {
                            document.activeElement.click();
                        } else {
                            alert("No Internet Connection");
        ...
        var el = document.createElement('div');
        el.id = 'loadingbar';
        el.style = 'display: flex;width: 100%; height: 100%; position: fixed; top: 0px;left: 0px;align-items: center; justify-content: center;background-color: black;';
        el.innerHTML = '<div style="display: inline-block; padding: 12px; background: white; font-weight: ;border: 5px solid lime; width: 70%"><center><img src="icons/player.png" width="56px"></center><br>S-Video Player Required<br><br><small><strong>Why?</strong><br><br>1. To Play .mkv Files<br>2. To Run This App</small>';
        document.body.appendChild(el);
    }
});